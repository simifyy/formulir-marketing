...
function App() {
  ...
  const [step, setStep] = useState(() => {
    const savedStep = localStorage.getItem('halalFormStep_vFinal2');
  ...
  const [formData, setFormData] = useState(() => {
    const savedData = localStorage.getItem('halalFormData_vFinal2');
    return savedData ? JSON.parse(savedData) : {
      ...
      namaUsaha: '',
      alamatDetail: {
        rtrw: '', kelurahan: '', kecamatan: '', kota: '', kodepos: '', provinsi: ''
      },
  ...
  useEffect(() => {
    ...
    localStorage.setItem('halalFormData_vFinal2', JSON.stringify(dataToSave));
    localStorage.setItem('halalFormStep_vFinal2', step.toString());
  }, [formData, step]);

  const selectStyles = {
    control: base => ({ ...base, borderColor: '#ccc', padding: '6px', borderRadius: '6px' }),
    option: (base, state) => ({
      ...base, backgroundColor: state.isSelected ? '#2e7d32' : 'white', color: state.isSelected ? 'white' : '#000', cursor: 'pointer'
    })
  };
  ...
  const handleAlamatChange = (e) => {
    setFormData({
      ...formData,
      alamatDetail: { ...formData.alamatDetail, [e.target.name]: e.target.value }
    });
  };
  ...
  const handleNIBChange = (e) => {
    setFormData({
      ...formData,
      nibDetail: { ...formData.nibDetail, [e.target.name]: e.target.value }
    });
  };
  ...
  const addProduk = () => {
    if (formData.produkList.length < 7) {
      setFormData({
        ...formData,
        produkList: [...formData.produkList, { nama: '', fotoProduk: null, urlProduk: '', fotoPendamping: null, urlPendamping: '' }]
      });
    } else {
      alert("Maksimal 7 Produk.");
    }
  };
  ...
  const handleNext = () => {
    if (!formData.marketing) return alert("⚠️ Pilih Pendamping dulu.");
    if (!formData.persetujuan) return alert("⚠️ Anda harus mencentang persetujuan!");
    if (!formData.nama || !formData.nik || !formData.wa) return alert("⚠️ Lengkapi Data Diri.");
    setStep(2);
    window.scrollTo(0, 0);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.namaUsaha) return alert("Isi Nama Usaha");
    const ad = formData.alamatDetail;
    if(!ad?.rtrw || !ad?.kelurahan || !ad?.kecamatan || !ad?.kota || !ad?.provinsi) return alert("Lengkapi Alamat Usaha");
    if (!formData.produkList || formData.produkList.length === 0 || !formData.produkList[0].nama) return alert("Minimal isi 1 produk");
    if(!formData.fotoKTP) return alert("Wajib upload Foto KTP");

    if (formData.statusNIB === 'punya') {
      if(!formData.nibDetail?.nomor || !formData.nibDetail?.email || !formData.nibDetail?.password) return alert("Lengkapi Data NIB");
    }

    setStatus('loading'); 

    const urlKTP = await uploadToHosting(formData.fotoKTP);
    if (!urlKTP) { setStatus(''); return alert("Gagal Upload KTP"); }
    ...
    try {
      ...
      setStatus('success');
      localStorage.removeItem('halalFormData_vFinal2');
      localStorage.removeItem('halalFormStep_vFinal2');
    } catch (error) {
      console.error(error);
      setStatus('error');
    }
        ...
        <form onSubmit={handleSubmit}>
          {step === 1 && (
            <>
              ...
              {/* PERNYATAAN 2 POIN SEBELUM CEKLIS */}
            ...
            <>
              ...
              <div className="form-group">
                ...
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
                  <input className="input-field" placeholder="RT/RW" name="rtrw" value={formData.alamatDetail?.rtrw || ''} onChange={handleAlamatChange} />
                  <input className="input-field" placeholder="Kelurahan/Desa" name="kelurahan" value={formData.alamatDetail?.kelurahan || ''} onChange={handleAlamatChange} />
                  <input className="input-field" placeholder="Kecamatan" name="kecamatan" value={formData.alamatDetail?.kecamatan || ''} onChange={handleAlamatChange} />
                  <input className="input-field" placeholder="Kota/Kab" name="kota" value={formData.alamatDetail?.kota || ''} onChange={handleAlamatChange} />
                  <input className="input-field" placeholder="Provinsi" name="provinsi" value={formData.alamatDetail?.provinsi || ''} onChange={handleAlamatChange} />
                  <input className="input-field" placeholder="Kode Pos" name="kodepos" value={formData.alamatDetail?.kodepos || ''} onChange={handleAlamatChange} />
                </div>
              </div>
              ...
              <div className="form-group" style={{background:'#f9f9f9', padding:'15px', borderRadius:'8px', border:'1px solid #eee'}}>
